...
-- * InsertService:LoadAsset → More reliable than game:GetObjects for tools.
-- * Handle naming + RequiresHandle → Guns expect a part named exactly "Handle".
-- * Script enabling + warnings → LocalScripts often need Disabled = false.
-- * Better timing/delays → Gives replication and character loading time.
-- * Cleanup → Avoids memory leaks from loaded assets.
-- * Extra equip/activate loops for guns that need it.
-- =========================================================================
...
local function giveTool(toolId)
    ...
    local success, result = pcall(function()
        ...
        -- Use InsertService for better reliability
        ...
        -- Deep search for Tool (handles models/containers)
        ...
        if not handle then
            ...
            if handle then
                handle.Name = "Handle"  -- Critical for many guns
        ...
        -- Enable all scripts (especially LocalScripts)
        ...
        if humanoid then
            -- Force equip with delay
            humanoid:EquipTool(tool)
            print("✅ Equipped: " .. tool.Name)

            task.wait(0.3)

            -- Multiple activation attempts for guns
            for i = 1, 3 do
                pcall(function()
                    tool:Activate()
                end)
...
-- ==================== GUI Setup (unchanged core, minor tweaks) ====================
...
print("✅ Improved Tool giver loaded (cxzpher)")