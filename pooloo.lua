-- ✅ TOOL GIVER GUI v3.1 - Fixed for HyperlaserGun & most weapons
...
local function giveTool(toolId)
    ...
    local success, result = pcall(function()
        ...
        -- Find the Tool (may be inside a Model)
        ...
        -- Fix for guns/weapons
        local handle = tool:FindFirstChild("Handle") or tool:FindFirstChildWhichIsA("BasePart")
        if handle then
            tool.RequiresHandle = true
            -- Do NOT do tool.Handle = handle (causes error on many tools)
        end
        ...
        if humanoid and tool.Parent == player.Backpack then
            ...
            task.wait(0.15)
            pcall(function() tool:Activate() end)  -- Safe activate for guns
        end
...
subtitle.Text = "Paste Tool ID → Works with most guns"
...
ToolBox.PlaceholderText = "130113146 (HyperlaserGun example)"
...
print("✅ Tool Giver GUI v3.1 loaded — Fixed handle error")