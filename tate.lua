...
local function sendGlobalMessage()
    ...
    -- Multiple methods for maximum visibility
    pcall(function()
    ...
    -- Try to fire common chat remotes if they exist (for extra reach)
    pcall(function()
        ...
        for _, v in ipairs(replicated:GetDescendants()) do
            if v:IsA("RemoteEvent") and (v.Name:lower():find("chat") or v.Name:lower():find("message")) then
                v:FireServer("[SERVER] " .. msg)
            end
        end
    end)

    print("✅ Global message sent: " .. msg)
end
