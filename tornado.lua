...
local function spawnTornado(position)
    local success, err = pcall(function()
        ...
        local tornadoModel = objects[1]
        
        if not tornadoModel then
            warn("❌ Failed to load tornado model")
            return
        end
        
        -- Clone and prepare
        tornadoModel = tornadoModel:Clone()
        tornadoModel:PivotTo(CFrame.new(position) * CFrame.new(0, 5, 0)) -- spawn slightly above ground
        
        -- Important: Parent to Workspace on the server for replication + damage
        -- (Since this is LocalScript, we fire a Remote or just parent – but for FE games use Remote)
        -- For most games this works directly if FilteringEnabled allows, but better to use server spawn if possible.
    ...
    if not success then
        warn("❌ Tornado spawn failed: " .. tostring(err))
    end
end
...
local function startPlacement()
    ...
    mouse.Icon = "rbxasset://textures/GunCursor.png" -- optional visual feedback
    
    connection = UserInputService.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            local target = mouse.Hit
            if target then
                spawnTornado(target.Position)
                -- Optional: stop after one spawn
                stopPlacement()
            end
        end
    end)
    
    print("🖱️ Click on the ground to spawn tornado!")
end
...
subtitle.Text = "Click Spawn → Click ground to place"
...
SpawnBtn.Position = UDim2.new(0.1, 0, 0, 90)
...
Status.Text = "Ready — Works for everyone + damage"
...
print("✅ Tornado Spawner loaded (cxzpher) - Click the button then click ground")